"""Expanding-window walk-forward validation splits. Never a random split across player-game
rows -- every fold's training set is strictly earlier seasons than its validation season.
Ported verbatim from the sibling cfb_win_total_model/cfb_spread_model projects' modeling/
splits.py -- this logic is target-agnostic (season-boundary bookkeeping only) and appropriate
unmodified at player-week grain: leakage protection lives entirely in the per-row lag step
(features/rolling_utils.py, eligibility.py's merge_asof), not the fold structure, so the fold
only needs to guarantee no SEASON crosses the train/validation boundary -- exactly as today.
"""
